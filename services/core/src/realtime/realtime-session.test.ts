...
describe("RealtimeSession", () => {
  ...
  it("closes an integrated session when a snapshot failure marker is cleared", async () => {
    const sessionId = randomUUID();
    const epochManager = new ContextEpochManager({ generateId: randomUUID });
    const now = new Date("2026-09-16T00:00:00.000Z");
    const epoch = epochManager.acceptUserInput(now);
    ...
    await ledger.append({
      content: "Retryable earlier question",
      contextEpoch: epoch,
      id: randomUUID(),
      occurredAt: now,
    ...
    await ledger.markRequestFailed(failedRequestId, epoch.id, now);
    await ledger.append({
    ...
    await ledger.clearRequestFailure(failedRequestId);
    ...
    const output = await collect(
      session.handle({
        ...
        sequence: 2,
        sessionId,
        text: "Must reconnect after retry state changes",
        turnId: randomUUID(),
        type: "input.text",
      }),
    );
  ...
  it("suppresses an automatic VAD response when another entry point advances the epoch", async () => {
    const sessionId = randomUUID();
    const clientAudioTurnId = randomUUID();
      ...
      conversationPort: {
        async open() {
          return {
            ...
            async close() {},
            async *outputs() {
              yield { responseId, turnId: providerTurnId, type: "response-started" } as const;
              yield {
                responseId,
                text: "Stale answer",
  ...
  it.each([
    {
      closesSession: false,
      name: "cancelled",
      terminal: (responseId: string) => ({ responseId, type: "response-cancelled" }) as const,
    },
    {
      closesSession: true,
      name: "provider error",
    ...
    "marks a persisted realtime turn failed after a $name response",
    async ({ closesSession, terminal }) => {
      const sessionId = randomUUID();
      const turnId = randomUUID();
      const responseId = randomUUID();
      const ledger = new InMemoryConversationLedger();
      ...
      await collect(
        session.handle({
          ...
          sequence: 2,
          sessionId,
          text: "Persist this terminal turn",
          turnId,
          type: "input.text",
        }),
      );

      await collect(session.outputs());
      ...
      expect(session.closed).toBe(closesSession);
      expect(closeProvider).toHaveBeenCalledTimes(closesSession ? 1 : 0);
    },
  );

  it("retries a transient failure while persisting a terminal realtime turn", async () => {
    const sessionId = randomUUID();
    const turnId = randomUUID();
    const responseId = randomUUID();
    const ledger = new InMemoryConversationLedger();
    ...
    const markFailed = vi
      ...
      .mockImplementation(persistFailure);
    const session = new RealtimeSession({
      conversationEndIntent: neverEndsConversation,
      conversationPort: {
        async open() {
    ...
    await collect(
      session.handle({
        ...
        sequence: 2,
        sessionId,
        text: "Persist a terminal turn despite one transient failure",
        turnId,
        type: "input.text",
      }),
    );
  ...
  it("allows an integrated session to reopen a failed turn from its initial snapshot", async () => {
    const sessionId = randomUUID();
    const turnId = randomUUID();
    const now = new Date("2026-09-16T00:00:00.000Z");
    const epochManager = new ContextEpochManager({ generateId: randomUUID });
    const contextEpoch = epochManager.acceptUserInput(now);
    const ledger = new InMemoryConversationLedger();
    const outputQueue = new AsyncQueue<RealtimeConversationOutput>();
    const responseId = randomUUID();
    await ledger.append({
      content: "Retry across sessions",
      contextEpoch,
      id: randomUUID(),
      occurredAt: now,
      requestId: turnId,
    ...
    await ledger.markRequestFailed(turnId, contextEpoch.id, now);
    const sent: RealtimeConversationInput[] = [];
    const session = new RealtimeSession({
      conversationEndIntent: neverEndsConversation,
      conversationPort: {
        async open() {
          return {
            ...
            async send(input) {
              sent.push(input);
              if (input.type !== "text" || sent.length !== 1) return;
              outputQueue.push({
                responseId,
                turnId: input.turnId,
                type: "response-started",
    ...
    expect(sent).toMatchObject([
      { text: "Retry across sessions", turnId, type: "text" },
      { text: "Continue after retry", turnId: nextTurnId, type: "text" },
    ]);
    await expect(ledger.listTurns({ contextEpochId: contextEpoch.id })).resolves.toMatchObject([
      { completed: true, failed: false, requestId: turnId },
      { completed: false, failed: false, requestId: nextTurnId },
    ]);
  });

  it("rejects a failed turn retried with different content", async () => {
    const sessionId = randomUUID();
    const turnId = randomUUID();
    const ledger = new InMemoryConversationLedger();
    let sendAttempts = 0;
    ...
    await expect(
      ...
      {
        code: "REALTIME_TURN_CONTENT_MISMATCH",
        type: "error",
      },
    ]);
    expect(sendAttempts).toBe(1);
    const user = await ledger.findByRequest(turnId, "user");
    await expect(
      ledger.listTurns({ contextEpochId: user?.contextEpochId ?? "" }),
    ).resolves.toMatchObject([{ completed: false, failed: true, requestId: turnId }]);
  });

  it("releases the turn claim when the user event append fails", async () => {
    const sessionId = randomUUID();
    const turnId = randomUUID();
    const ledger = new InMemoryConversationLedger();
    const append = ledger.append.bind(ledger);
    let failUserAppend = true;
    vi.spyOn(ledger, "append").mockImplementation((input) => {
      if (failUserAppend && input.role === "user") {
        failUserAppend = false;
        return Promise.reject(new Error("Temporary user append failure"));
      }
      return append(input);
    });
    const sent: RealtimeConversationInput[] = [];
    ...
    await expect(
      collect(
        session.handle({
          ...
          sequence: 2,
          sessionId,
          text: "Retry persistence",
          turnId,
          type: "input.text",
        }),
      ),
    ).rejects.toThrow("Temporary user append failure");
    await expect(
      collect(
        session.handle({
          eventId: randomUUID(),
    ...
    expect(sent).toMatchObject([{ text: "Retry persistence", turnId, type: "text" }]);
  });

  it("suppresses every provider output when the turn claim is rejected", async () => {
    const sessionId = randomUUID();
    const turnId = randomUUID();
    const responseId = randomUUID();
    const ledger = new InMemoryConversationLedger();
    ...
    await expect(collect(session.outputs())).resolves.toEqual([]);
    await expect(ledger.list()).resolves.toMatchObject([
      { content: "Unclaimed transcript", requestId: turnId, role: "user" },
    ]);
  });
  ...
  it("recovers terminal failure persistence after immediate retries are exhausted", async () => {
    ...
    await collect(
      session.handle({
        ...
        sequence: 2,
        sessionId,
        text: "Do not leak the provider connection",
        turnId,
        type: "input.text",
      }),
    );
  ...
  it("clears response mappings after a scoped provider error", async () => {
    ...
    const responseId = randomUUID();
    const ledger = new InMemoryConversationLedger();
    const markFailed = vi.spyOn(ledger, "markRequestFailed");
    const session = new RealtimeSession({
      conversationEndIntent: neverEndsConversation,
      conversationPort: {
        async open() {
  ...
  it("does not fail another open turn when a provider error identifies its turn", async () => {
    const sessionId = randomUUID();
        ...
        async open() {
          return {
            ...
            async *outputs() {
              yield {
                code: "PROVIDER_FAILED",
                message: "Only the first turn failed",
                retryable: true,
                turnId: failedTurnId,
                type: "error",
              } as const;
    ...
    for (const [sequence, turnId, text] of [
      [2, failedTurnId, "First concurrent turn"],
      [3, completedTurnId, "Second concurrent turn"],
    ] as const) {
      await collect(
        session.handle({
          eventId: randomUUID(),
    ...
    await expect(
      ...
      { completed: false, failed: true, requestId: failedTurnId },
      { completed: true, failed: false, requestId: completedTurnId },
    ]);
  });

  it("does not terminalize a turn for a nonterminal unscoped provider error", async () => {
    const sessionId = randomUUID();
    const turnId = randomUUID();
    const ledger = new InMemoryConversationLedger();
    const session = new RealtimeSession({
      ...
      conversationPort: {
        async open() {
          return {
            ...
            async *outputs() {
              yield {
                code: "UNRELATED_PROVIDER_ERROR",
                message: "An unrelated provider operation failed",
                retryable: true,
                type: "error",
              } as const;
            },
  ...
  it("applies an early automatic-VAD cancellation after the final transcript is persisted", async () => {
    const sessionId = randomUUID();
    const clientAudioTurnId = randomUUID();
    const providerTurnId = randomUUID();
    const responseId = randomUUID();
    ...
    expect(output.map((event) => event.type)).toEqual(["response.cancelled", "input.transcript"]);
    await expect(
      ledger.listTurns({ contextEpochId: user?.contextEpochId ?? "" }),
    ).resolves.toMatchObject([
      {
  ...
  it("applies an automatic-VAD provider error before response start to the late transcript", async () => {
    const sessionId = randomUUID();
    const clientAudioTurnId = randomUUID();
    const providerTurnId = randomUUID();
    const ledger = new InMemoryConversationLedger();
      ...
      conversationPort: {
        async open() {
          return {
            ...
            async close() {},
            async *outputs() {
              yield { turnId: providerTurnId, type: "speech-started" } as const;
              yield {
                code: "PROVIDER_FAILED",
                message: "Provider failed before creating a response",
                retryable: true,
                turnId: providerTurnId,
                type: "error",
              } as const;
              yield {
                final: true,
                text: "Question transcribed after the provider error",
                turnId: providerTurnId,
                type: "transcript",
              } as const;
            },
  ...
  it("does not fail an active turn when sending its cancellation is rejected", async () => {
    const sessionId = randomUUID();
    const turnId = randomUUID();
    const responseId = randomUUID();
    let continueResponse = () => {};
    const responseStarted = new Promise<void>((resolveStarted) => {
      continueResponse = resolveStarted;
    });
    let releaseCompletion = () => {};
    const completionReleased = new Promise<void>((resolveCompletion) => {
        ...
        async open() {
          return {
            ...
            async *outputs() {
              ...
              yield {
                responseId,
                text: "Original response completed",
                turnId,
                type: "response-text",
              } as const;
              yield {
    ...
    await collect(
      session.handle({
        ...
        sequence: 2,
        sessionId,
        text: "Keep the original response alive",
        turnId,
        type: "input.text",
      }),
    );
    ...
    const cancelResult = await collect(
      session.handle({
        eventId: randomUUID(),
        responseId,
        sequence: 3,
    ...
    const user = (await ledger.list())[0];
    expect(cancelResult).toMatchObject([{ code: "REALTIME_INPUT_FAILED", type: "error" }]);
    await expect(
      ledger.listTurns({ contextEpochId: user?.contextEpochId ?? "" }),
    ).resolves.toMatchObject([{ completed: false, failed: false, requestId: turnId }]);

    releaseCompletion();
    const completedOutput = await outputTask;
    expect(completedOutput.map((event) => event.type)).toEqual([
      "response.started",
      "response.text",
      "response.completed",
    ]);
  ...
  it("applies a failed manual commit to a final transcript that arrives later", async () => {
    const sessionId = randomUUID();
    const turnId = randomUUID();
    const ledger = new InMemoryConversationLedger();
    const session = new RealtimeSession({
        ...
        async open() {
          return {
            ...
            async *outputs() {
              yield {
                final: true,
                text: "Transcript from the accepted audio commit",
                turnId,
                type: "transcript",
              } as const;
            },
            async send(input) {
              if (input.type === "commit") {
                throw new Error("Response creation failed after audio commit");
              }
            },
          };
        },
    ...
    const commitResult = await collect(
      session.handle({
        eventId: randomUUID(),
        sequence: 3,
        sessionId,
    ...
    expect(commitResult).toMatchObject([{ code: "REALTIME_INPUT_FAILED", type: "error" }]);
    await collect(session.outputs());
    const user = (await ledger.list())[0];
    await expect(
      ledger.listTurns({ contextEpochId: user?.contextEpochId ?? "" }),
