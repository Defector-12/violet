...
- Release 1C.1 Natural Pointing 已形成按需视觉候选：不在唤醒时预截图，按当前 `turnId` 使用 AX 或单显示器截图，DeepSeek 按用户问题作答，Core 校验后交给 Qwen。终端曾错误放行相邻命令；`c2ea558-candidate` 现以鼠标所在连通高亮紧裁图约束目标，固定样本未再误选相邻命令，但逐字稳定率和超时仍未达到完整发布门槛。当前部署与测试统一见 [Release 1C 验收](./release-1c-acceptance.md)。
