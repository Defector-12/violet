...
记忆不是聊天记录，也不能把向量相似度当成真实性。长期演进时可以从以下认知维度理解
内容，但这些维度不是 Release 1D 的数据库分区或必须同时实现的记忆系统：
...
每条记忆必须保留来源、时间、类型、证据状态、敏感级别、版本和影响范围。核心少量信息
进入有界摘要，其余内容按当前意图检索；任何用户编辑和删除都必须传播到索引与推断。
...
- Release 1D 先使用 Core 内的规范化文本与时间检索；只有固定验收集证明召回率不达标
  且根因在检索时，才评估 `pgvector`，不预建双检索链路。
- 加密对象存储保存经过授权的关键截图、文件与交付物。
- 原始连续感知数据采用短期、可配置留存，不作为长期记忆事实源。

Release 1D 的原子记忆、上下文预算、内部 epoch、检索和管理合同见
[Release 1D 最终规格](./release-1d-spec.md)。
...
2. 请求被接受后立即停止检索、推理和工具使用；在线事实、派生状态和详细观测不得继续返回目标内容。
3. 审计层只保留删除 ID、时间、发起设备、作用域类别、执行状态和策略版本，不保留原文、摘要、文件名、对象键或可恢复语义。最小审计元数据保留至整个 Violet 实例被删除。
4. 每次内容删除单调增加恢复纪元；Mac Keychain 保存最低允许纪元，Violet 官方恢复在
   解密前和 Core 启动前拒绝更旧的备份。
5. 删除后生成并验证不含目标内容的新备份，再清理 Violet 管理的旧本地副本及对象存储
   的旧版本、删除标记和未完成分片。清理失败不能解除在线删除或降低最低恢复纪元。
6. 删除整个 Violet 实例时，销毁内容、审计、对象、所有备份及根密钥，不保留删除墓碑。
...
- 记忆检索 ID、来源状态、是否采用和删除传播状态，不记录记忆正文。
...
- AX 不可用时需要截图兜底；几何校验不等于文字正确性验证。保留失败证据，
  先由开发者回放，再交最终人工验收。
- Release 1D 的记忆设计已冻结在
  [Release 1D 最终规格](./release-1d-spec.md)；实现结果仍须通过专项验收。
