...
- Release 1C.1 Natural Pointing 候选已部署：用户显式开启 `Look` 后，Mac 在浮窗夺取焦点前保存当前应用、选区和鼠标位置，优先读取选中文字，否则捕获前台窗口。Realtime 先使用本地证据建立会话，复杂视觉问题通过 Core 内部只读 Context 工具等待后台 Vision。该能力不连续采帧，也不扩大未唤醒时的感知范围；当前仍待真实环境验收。
...
- 当前 1C.1 本地候选的格式、生成物、类型和构建门禁通过，TypeScript/JavaScript 79 个测试和 Swift 36 个测试通过；Mac App 已完成打包与签名校验。本地唤醒模型使用合成 `Violet` 音频完成 100 次正样本和 100 次静音负样本验证，结果为 100/100 触发、0/100 误触发，CPU 处理 p95 为 22.2ms；该结果不能替代真实办公环境验收。Release 1A 的真实模型 20 轮纵向验证、两次物理重启、加密备份、TOS 上传下载和空库恢复已经通过，Release 1B Core 的 ready/sealed、认证、Realtime 握手和遥测白名单验证通过。2026-08-27 收口前 Devbox 数据库包含 567 条连续加密事件。
...
Release 1C.1 增加独立且默认关闭的 `Look` 授权。它只在一次语音唤醒时建立最长 5 分钟
的 Context，不持续采帧：Mac 在浮窗夺取焦点前保存选区、前台应用和鼠标位置，优先发送
选中文字，否则捕获前台窗口并附带归一化焦点坐标。图片先经过本地隐私门禁，再由 Core
后台理解；Realtime 通过只读 Context 工具按需等待完整结果。关闭、锁屏、睡眠、撤权或
超时仍按既有生命周期删除 Context。
...
按《工程路线》执行第一阶段。Release 1A 基座与 Release 1B Presence 已完成；Qwen 是默认实时运行时，Pipeline 是显式配置的降级与替换基线，禁止静默自动切换。Release 1C Violet Sight 的代码已集成到主线，剩余视觉、隐私生命周期和唤醒验收按用户决定暂停并保持可追踪，Release 状态仍是实现候选。Release 1C.1 Natural Pointing 候选已部署，下一步完成真实冒烟；随后进入 Release 1D Violet Continuity。1C 与 1C.1 的完整验收在后续主功能完成后统一恢复，也可由用户随时发起。
