...
功能仍未通过验收。终端已多次真实复测，包括错误放行相邻命令；不能将结构门禁通过
当成正确识别。当前部署、回放结果和剩余门禁统一见 [Release 1C 验收](./release-1c-acceptance.md)。
...
- 本地删除了硬编码 HTTP 取证插桩；私有失败样本保留供当前问题回放，未全部清理。
- 新候选尚未部署，不要把旧部署状态复制到新代码的验收结论。
